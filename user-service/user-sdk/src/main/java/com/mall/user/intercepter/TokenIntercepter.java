...
public class TokenIntercepter extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request,HttpServletResponse response,Object handler) throws Exception {
        ...
        try {
            checkAuthResponse = loginService.validToken(checkAuthRequest);
        } catch (Exception e) {
            ResponseData responseData=new ResponseUtil().setErrorMsg(SysRetCodeConstants.SYSTEM_ERROR.getMessage());
            response.setContentType("text/html;charset=UTF-8");
            response.getWriter().write(JSON.toJSON(responseData).toString());
            return false;
        }
